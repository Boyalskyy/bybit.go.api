...
func (b *WebSocket) handleIncomingMessages() {
	for {
		...
		if err != nil {
			log.Printf("bybit ws: error reading: %v", err)
			b.isConnected = false
			return
		}
		...
		if b.onMessage != nil {
			...
			if err != nil {
				log.Printf("bybit ws: error handling message: %v", err)
				return
			}
		}
	}
...
func (b *WebSocket) monitorConnection() {
	...
	for {
		...
		if !b.isConnected && b.ctx.Err() == nil { // Check if disconnected and context not done
			log.Println("bybit ws: attempting to reconnect...")
			con := b.Connect() // Example, adjust parameters as needed
			if con == nil {
				log.Println("bybit ws: reconnection failed")
			} else {
				b.isConnected = true
...
func (b *WebSocket) Connect() *WebSocket {
	...
	if b.requiresAuthentication() {
		if err = b.sendAuth(); err != nil {
			log.Printf("bybit ws: connection failed: %v", err)
			return nil
		}
	}
	b.isConnected = true
...
func ping(b *WebSocket) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				log.Printf("bybit ws: failed to marshal ping: %v", err)
				continue
			}
			if err := b.conn.WriteMessage(websocket.TextMessage, jsonPingMessage); err != nil {
				log.Printf("bybit ws: failed to send ping: %v", err)
				return
			}
